...
single_pass_ktuned_DMA_manual_specialized_one_phase(float2 *g_PnQn,
                   float2* g_PQ, 
#ifndef USE_TEX				   
...
//		dma_ld_pq1.wait_for_dma_finish();
	...
	else if(dma_ld_pq1.owns_this_thread())
	{
		int in_idx = gid - R_x_row_stride - R 
						 - R*slice_stride;
#ifdef DYNAMIC
		for(int i=0; i<(2*R+nz); i+=2)
#else
                #pragma unroll 1
                for(int i=0; i<(2*R+RTM_ELMTS); i+=2)
#endif
		{
			dma_ld_pq1.execute_dma(&g_PQ[in_idx],s_PQ1);
			in_idx += slice_stride;
			dma_ld_pq2.execute_dma(&g_PQ[in_idx],s_PQ2);
			in_idx += slice_stride;
		}
	}
}
