...
single_pass_ktuned_DMA_manual_specialized_two_phase(float2 *g_PnQn,
                   float2* g_PQ, 
#ifndef USE_TEX				   
    ...
    )
{
	...
	if(tid<COMPUTE_THREADS_PER_CTA)
	{
		...
		int z_base = 0;
#ifdef DYNAMIC
		for(int iz=0; iz<(nz-2); iz+=2)
#else
                #pragma unroll 1
                for(int iz=0; iz<(RTM_ELMTS-2); iz+=2)
#endif
		{
			//------------------------------------------------------
			// Phase 1
...
#ifdef DYNAMIC
                for(int i=0; i<(2*R+nz-2); i+=2)
#else
                #pragma unroll 1
		for(int i=0; i<(2*R+RTM_ELMTS-2); i+=2)
#endif
		{
