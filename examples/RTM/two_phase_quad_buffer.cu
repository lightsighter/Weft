...
single_pass_ktuned_DMA_quad_specialized_two_phase(float2 *g_PnQn,
                   float2* g_PQ, 
#ifndef USE_TEX				   
...
{
	int tid = threadIdx.x;
	__shared__ float2 s_PQ1[2*(TILE_Y+2*R)*COUNT_PER_STRIDE_ROW+TILE_Y*COUNT_PER_STRIDE_ROW];
	float2 *s_PQ2   = s_PQ1+  (TILE_Y+2*R)*COUNT_PER_STRIDE_ROW;
	float2 *PQy_buf = s_PQ1+2*(TILE_Y+2*R)*COUNT_PER_STRIDE_ROW+PQY_BUF_OFFSET;
	cudaDMAStridedTwoPhase<true,float4,16,LDG_BYTES,COUNT_PER_STRIDE_ROW*sizeof(float2),DMA_THREADS_PER_LD,TILE_Y+2*R>
		dma_ld_pq1(1,
	...
	// Note aliased buffers and shared memory
	cudaDMAStridedTwoPhase<true,float4,16,LDG_BYTES,COUNT_PER_STRIDE_ROW*sizeof(float2),DMA_THREADS_PER_LD,TILE_Y+2*R> 
		dma_ld_pq3(1,
				COMPUTE_THREADS_PER_CTA,
				COMPUTE_THREADS_PER_CTA,
	...
	if(tid<COMPUTE_THREADS_PER_CTA)
	{
		...
		Q_DEF (q_Qyy, float);
		Q_DEF (q_Qxy, float);
		//SMEM_Q_DEF (q_Qxx, float);
		//SMEM_Q_DEF (q_Qyy, float);
		//SMEM_Q_DEF (q_Qxy, float);

		//======================================================
		// Prime the pump
		//======================================================
		dma_ld_pq1.start_async_dma();
		dma_ld_pq2.start_async_dma();
		for(int i=0; i<2*R; i+=4)
		{
			//------------------------------------------------------
			// Phase 1
			//------------------------------------------------------
			...
			dma_ld_pq1.wait_for_dma_finish(); // s_PQ1 ready, synchronization point
			l_P[ZPENCIL_LAST] = s_PQ1[PQ_index].x;
			l_Q[ZPENCIL_LAST] = s_PQ1[PQ_index].y;

			// Compute x, xx stencils
			float q_Pxxl, q_Qxxl;
			l_Px[ZPENCIL_LAST] = c_x[0]*l_P[ZPENCIL_LAST];
        	l_Qx[ZPENCIL_LAST] = c_x[0]*l_Q[ZPENCIL_LAST];
        	q_Pxxl = c_xx[0]*l_P[ZPENCIL_LAST];
        	q_Qxxl = c_xx[0]*l_Q[ZPENCIL_LAST];
			#pragma unroll
			for(int j=1;j<=R;j++) {
				const float2 v1 = s_PQ1[PQ_index-j];
				const float2 v2 = s_PQ1[PQ_index+j];
			...
			float2 PQyside;
			PQyside.x = c_y[0] * s_PQ1[PQ_index+PQ_halo_diff].x; 
			PQyside.y = c_y[0] * s_PQ1[PQ_index+PQ_halo_diff].y; 
			#pragma unroll
			for(int j=1;j<=R;j++)
			{
				const float2 v1 = s_PQ1[PQ_index+PQ_halo_diff-j*COUNT_PER_STRIDE_ROW];
				const float2 v2 = s_PQ1[PQ_index+PQ_halo_diff+j*COUNT_PER_STRIDE_ROW];
			...
			#pragma unroll
			for(int j=1;j<=R;j++) {
				const float2 v1 = s_PQ1[PQ_index-j*COUNT_PER_STRIDE_ROW];
				const float2 v2 = s_PQ1[PQ_index+j*COUNT_PER_STRIDE_ROW];
			...
			dma_ld_pq3.start_async_dma(); // fetch next s_PQ1
			...
			dma_ld_pq2.wait_for_dma_finish(); // s_PQ2 ready, synchronization point
			l_P[ZPENCIL_LAST] = s_PQ2[PQ_index].x;
			l_Q[ZPENCIL_LAST] = s_PQ2[PQ_index].y;
			...
			#pragma unroll
			for(int j=1;j<=R;j++) {
				const float2 v1 = s_PQ2[PQ_index-j];
				const float2 v2 = s_PQ2[PQ_index+j];
				l_Px[ZPENCIL_LAST] += c_x[j] * (v1.x + v2.x);
				l_Qx[ZPENCIL_LAST] += c_x[j] * (v1.y + v2.y);
				q_Pxxl += c_xx[j] * (v1.x + v2.x);
				q_Qxxl += c_xx[j] * (v1.y + v2.y);
			...
			// Compute y stencils for halo
			PQyside.x = c_y[0] * s_PQ2[PQ_index+PQ_halo_diff].x; 
			PQyside.y = c_y[0] * s_PQ2[PQ_index+PQ_halo_diff].y; 
			#pragma unroll
			for(int j=1;j<=R;j++)
			{
				const float2 v1 = s_PQ2[PQ_index+PQ_halo_diff-j*COUNT_PER_STRIDE_ROW];
				const float2 v2 = s_PQ2[PQ_index+PQ_halo_diff+j*COUNT_PER_STRIDE_ROW];
				PQyside.x += c_y[j] * (v1.x + v2.x);
				PQyside.y += c_y[j] * (v1.y + v2.y);
			}
			PQy_buf[PQ_index+PQ_halo_diff] = PQyside;
        	...
        	l_Py[ZPENCIL_LAST] = c_y[0]*l_P[ZPENCIL_LAST];
        	l_Qy[ZPENCIL_LAST] = c_y[0]*l_Q[ZPENCIL_LAST];
        	q_Pyyl = c_yy[0]*l_P[ZPENCIL_LAST];
        	q_Qyyl = c_yy[0]*l_Q[ZPENCIL_LAST];
			#pragma unroll
			for(int j=1;j<=R;j++) {
				const float2 v1 = s_PQ2[PQ_index-j*COUNT_PER_STRIDE_ROW];
				const float2 v2 = s_PQ2[PQ_index+j*COUNT_PER_STRIDE_ROW];
				l_Py[ZPENCIL_LAST] += c_y[j] * (v1.x + v2.x);
				l_Qy[ZPENCIL_LAST] += c_y[j] * (v1.y + v2.y);
				q_Pyyl += c_yy[j] * (v1.x + v2.x);
				q_Qyyl += c_yy[j] * (v1.y + v2.y);
			...
			ptx_cudaDMA_barrier_blocking(0,COMPUTE_THREADS_PER_CTA); // __syncthreads()
			dma_ld_pq4.start_async_dma(); // fetch next s_PQ2

			// Compute xy 
			q_Pxyl = c_x[0]*l_Py[ZPENCIL_LAST];
			q_Qxyl = c_x[0]*l_Qy[ZPENCIL_LAST];
			#pragma unroll
			for(int j=1;j<=R;j++)
			{
				const float2 v1 = PQy_buf[PQ_index-j];
			...
			dma_ld_pq3.wait_for_dma_finish(); // s_PQ1 ready, synchronization point
			l_P[ZPENCIL_LAST] = s_PQ1[PQ_index].x;
			l_Q[ZPENCIL_LAST] = s_PQ1[PQ_index].y;
			...
			ptx_cudaDMA_barrier_blocking(0,COMPUTE_THREADS_PER_CTA); // __syncthreads()
			dma_ld_pq1.start_async_dma(); // fetch next s_PQ1
			...
			dma_ld_pq4.wait_for_dma_finish(); // s_PQ2 ready, synchronization point
			l_P[ZPENCIL_LAST] = s_PQ2[PQ_index].x;
			l_Q[ZPENCIL_LAST] = s_PQ2[PQ_index].y;
			...
			ptx_cudaDMA_barrier_blocking(0,COMPUTE_THREADS_PER_CTA); // __syncthreads()
			dma_ld_pq2.start_async_dma(); // fetch next s_PQ2
...
#ifdef DYNAMIC
		for(int iz=0; iz<(nz-4); iz+=4)
#else
                #pragma unroll 1
                for(int iz=0; iz<(RTM_ELMTS-4); iz+=4)
#endif
		{
			//------------------------------------------------------
			// Phase 1
            ...
            ZPENCIL_SHIFT(l_Qx);
            ZPENCIL_SHIFT(l_Qy);
            float q_Pxx0 = Q_CURR(q_Pxx, qidx);
            float q_Qxx0 = Q_CURR(q_Qxx, qidx); 
            float q_Pyy0 = Q_CURR(q_Pyy, qidx);
            float q_Qyy0 = Q_CURR(q_Qyy, qidx); 
            float q_Pxy0 = Q_CURR(q_Pxy, qidx);
            float q_Qxy0 = Q_CURR(q_Qxy, qidx); 
            ADVANCE_6Qs(q_Pxx, q_Pyy, q_Pxy, q_Qxx, q_Qyy, q_Qxy, qidx);

			// Params
			REDUCTION_LD(float2 PnQn = ld_PnQn2_ro(out_idx));
			float4 abde = ld_param_ro(out_idx, abde);
			float  vpz2 = ld_param_ro(out_idx, vpz2);
			...
			#pragma unroll
            for(int i=1;i<=R;i++) {
				const float2 v1 = s_PQ1[PQ_index-i];
				const float2 v2 = s_PQ1[PQ_index+i];
	    ...
	    Q_LAST(q_Qxy, qidx) = q_Qxyl;
            ...
            float cosa2 = 1.f - sina2;
            float cosb2 = cosb * cosb;
            float sinb2 = 1.f - cosb2;
            float sin2a = 2.f * sina * cosa;
            float sin2b = 2.f * sinb * cosb;

            float h1p = sina*cosb2*q_Pxx0 + 
                sina2*sinb2*q_Pyy0 + 
                cosa2*Pzz + 
                sina2*sin2b*q_Pxy0 + 
                sin2a*sinb*Pyz + 
                sin2a*cosb*Pxz;
            float h2p = q_Pxx0 + q_Pyy0 + Pzz - h1p;

            float h1q = sina*cosb2*q_Qxx0 + 
                sina2*sinb2*q_Qyy0 + 
                cosa2*Qzz + 
                sina2*sin2b*q_Qxy0 + 
                sin2a*sinb*Qyz + 
                sin2a*cosb*Qxz;
            float h2q = q_Qxx0 + q_Qyy0 + Qzz - h1q;

            float vsz2 = vsz2_constant * fabsf(delta - epsln);
            float vpn2 = vpz2 * (1.f + 2.f*delta);
            float vpx2 = vpz2 * (1.f + 2.f*epsln);
	    REDUCTION_SUB(g_PnQn[out_idx].x, PnQn.x, 2.f*l_P[ZPENCIL_CTR_POSTSHIFT] + vpx2*h2p + vsz2*h1p + vpz2*h1q - vsz2*h1q);
            ...
            ZPENCIL_SHIFT(l_Qx);
            ZPENCIL_SHIFT(l_Qy);
            q_Pxx0 = Q_CURR(q_Pxx, qidx);
            q_Qxx0 = Q_CURR(q_Qxx, qidx); 
            q_Pyy0 = Q_CURR(q_Pyy, qidx);
            q_Qyy0 = Q_CURR(q_Qyy, qidx); 
            q_Pxy0 = Q_CURR(q_Pxy, qidx);
            q_Qxy0 = Q_CURR(q_Qxy, qidx); 
            ADVANCE_6Qs(q_Pxx, q_Pyy, q_Pxy, q_Qxx, q_Qyy, q_Qxy, qidx);

			// Params
			REDUCTION_LD(PnQn = ld_PnQn2_ro(out_idx));
			abde = ld_param_ro(out_idx, abde);
			vpz2 = ld_param_ro(out_idx, vpz2);
			...
			#pragma unroll
            for(int i=1;i<=R;i++) {
				const float2 v1 = s_PQ2[PQ_index-i];
				const float2 v2 = s_PQ2[PQ_index+i];
                l_Px[ZPENCIL_LAST] += c_x[i] * (v1.x + v2.x);
                l_Qx[ZPENCIL_LAST] += c_x[i] * (v1.y + v2.y);
                q_Pxxl += c_xx[i] * (v1.x + v2.x);
                q_Qxxl += c_xx[i] * (v1.y + v2.y);
	    ...
	    // Output calculation
            ...
            h1p = sina*cosb2*q_Pxx0 + 
                sina2*sinb2*q_Pyy0 + 
                cosa2*Pzz + 
                sina2*sin2b*q_Pxy0 + 
                sin2a*sinb*Pyz + 
                sin2a*cosb*Pxz;
            h2p = q_Pxx0 + q_Pyy0 + Pzz - h1p;

            h1q = sina*cosb2*q_Qxx0 + 
                sina2*sinb2*q_Qyy0 + 
                cosa2*Qzz + 
                sina2*sin2b*q_Qxy0 + 
                sin2a*sinb*Qyz + 
                sin2a*cosb*Qxz;
            h2q = q_Qxx0 + q_Qyy0 + Qzz - h1q;

            vsz2 = vsz2_constant * fabsf(delta - epsln);
            vpn2 = vpz2 * (1.f + 2.f*delta);
            vpx2 = vpz2 * (1.f + 2.f*epsln);
	    REDUCTION_SUB(g_PnQn[out_idx].x, PnQn.x, 2.f*l_P[ZPENCIL_CTR_POSTSHIFT] + vpx2*h2p + vsz2*h1p + vpz2*h1q - vsz2*h1q);
            REDUCTION_SUB(g_PnQn[out_idx].y, PnQn.y, 2.f*l_Q[ZPENCIL_CTR_POSTSHIFT] + vpz2*h1q + vsz2*h2q + vpn2*h2p - vsz2*h2p);
		...
		//------------------------------------------------------
		dma_ld_pq1.wait_for_dma_finish();
		dma_ld_pq2.wait_for_dma_finish();
		/*
		ZPENCIL_SHIFT(l_P);
		ZPENCIL_SHIFT(l_Px);
		ZPENCIL_SHIFT(l_Py);
...
//		dma_ld_pq1.wait_for_dma_finish();
	...
	else if(dma_ld_pq1.owns_this_thread())
	{
		int in_idx = gid - R_x_row_stride - R 
						 - R*slice_stride;
		dma_ld_pq1.begin_xfer_async(&g_PQ[in_idx]);
		in_idx += slice_stride;
		dma_ld_pq2.begin_xfer_async(&g_PQ[in_idx]);
		in_idx += slice_stride;
		dma_ld_pq3.begin_xfer_async(&g_PQ[in_idx]);
		in_idx += slice_stride;
		dma_ld_pq4.begin_xfer_async(&g_PQ[in_idx]);
		in_idx += slice_stride;

#ifdef DYNAMIC
                for(int i=0; i<(2*R+nz-8); i+=4)
#else
                #pragma unroll 1
		for(int i=0; i<(2*R+RTM_ELMTS-8); i+=4)
#endif
		{
			dma_ld_pq1.wait_xfer_commit(s_PQ1);
			dma_ld_pq1.begin_xfer_async(&g_PQ[in_idx]);
			in_idx += slice_stride;
			dma_ld_pq2.wait_xfer_commit(s_PQ2);
			dma_ld_pq2.begin_xfer_async(&g_PQ[in_idx]);
			in_idx += slice_stride;
			dma_ld_pq3.wait_xfer_commit(s_PQ1);
			dma_ld_pq3.begin_xfer_async(&g_PQ[in_idx]);
			in_idx += slice_stride;
			dma_ld_pq4.wait_xfer_commit(s_PQ2);
			dma_ld_pq4.begin_xfer_async(&g_PQ[in_idx]);
			in_idx += slice_stride;
		}
			dma_ld_pq1.wait_xfer_commit(s_PQ1);
